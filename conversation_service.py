...
def start_conversation(user_id, persona):
    logging.info(f"Starting conversation for user_id: {user_id}, persona: {persona}")
    conversation = Conversation(user_id=user_id, persona=persona)
    db.session.add(conversation)
    db.session.commit()
    logging.info(f"Conversation started with id: {conversation.id}")
...
async def generate_overall_feedback(conversation):
    #Here we have made changes
    language = session.get('language', 'Hindi')  # Default to Hindi if not set

    logging.info("feedback language",language)
    ...
    logging.debug(f"Overall response: {overall_response}")  # Add this line
    ...
    logging.debug(f"Raw overall feedback: {overall_feedback}")  # Add this line

    # Process the feedback to limit it to 2 points per category
    processed_feedback = process_feedback(overall_feedback)
    logging.debug(f"Processed feedback: {processed_feedback}")  # Add this line
    ...
    if language == 'Hindi':
        ...
        logging.debug(f"Translated feedback: {translated_chunk_text}")  # Add this line
...
async def llm_invoke(prompt):
    logging.debug(f"Sending prompt to LLM: {prompt[:100]}...")  # log only the first 100 chars to avoid overlogging
    response = await asyncio.to_thread(llm.invoke, prompt)
    logging.debug(f"Received response from LLM: {response.content[:100]}...")  # Same here
    return response
