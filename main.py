        ...
        async for line in f:
            ...
            if line:
                try:
                    ...
                    logging.warning(f"Ignoring invalid JSON data: {line}")
...
def read_persona_details_from_csv(csv_file):
    try:
        with open(csv_file, 'r', encoding='utf-8-sig') as file:
            reader = csv.DictReader(file)
            logging.debug(f"CSV Headers: {reader.fieldnames}")  # Debug headers for verification
            for row in reader:
                persona_name = row['Name'].strip().lower()
                persona_data1[persona_name] = {
                    'Age': row['Age'],
        ...
        logging.info(f"Completed loading personas from CSV. Total personas: {len(persona_data1)}")
    except Exception as e:
        logging.error(f"Error reading persona CSV file: {e}")
    return persona_data1
...
def load_refer_personas():
    ...
    logging.info("Successfully fetched referenced personas.")
...
def set_custom_persona():
    ...
    if (existing_persona):
        logging.warning(f"Persona with name '{name}' already exists for user {current_user.id}.")
    ...
    db.session.commit()
    logging.info(f"Custom persona '{name}' created successfully for user {current_user.id}.")
...
async def start_conversation1(persona_name):
    logging.info("Starting conversation for persona: %s", persona_name)
...
# code after changes for chat with predefined persona 
    persona = Persona.query.filter(func.lower(Persona.name) == persona_name.lower(),
                                   or_(Persona.user_id == current_user.id, Persona.user_id == None)
                                ).first()
    personas2 = Persona.query.filter(
        or_(Persona.user_id == current_user.id, Persona.user_id == None)
    ).all()
